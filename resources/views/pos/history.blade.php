    ...
    <div class="py-4 sm:py-6">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-3 sm:space-y-4">
            ...
            <div class="grid grid-cols-3 gap-2 sm:gap-4">
                <div class="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100 relative overflow-hidden">
                    <p class="text-[9px] sm:text-xs font-semibold text-gray-400 uppercase tracking-wider">Transaksi</p>
                    <h3 class="text-lg sm:text-2xl font-extrabold text-gray-800 mt-0.5">{{ $summary['total_trx'] }}</h3>
                    <p class="text-[9px] sm:text-xs text-gray-400 hidden sm:block">Pelanggan hari ini</p>
                    <div class="absolute right-0 top-0 h-full w-12 sm:w-16 bg-gradient-to-l from-indigo-50 to-transparent opacity-50"></div>
                </div>
                <div class="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100 relative overflow-hidden">
                    <p class="text-[9px] sm:text-xs font-semibold text-green-600 uppercase tracking-wider">Cash</p>
                    <h3 class="text-sm sm:text-2xl font-extrabold text-gray-800 mt-0.5">
                        <span class="text-[10px] sm:text-sm font-normal text-gray-400">Rp</span>
                        {{ number_format($summary['total_cash'], 0, ',', '.') }}
                    </h3>
                    <p class="text-[9px] sm:text-xs text-gray-400 hidden sm:block">Pendapatan Cash Hari Ini</p>
                    <div class="absolute right-0 top-0 h-full w-12 sm:w-16 bg-gradient-to-l from-green-50 to-transparent opacity-50"></div>
                </div>
                <div class="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-100 relative overflow-hidden">
                    <p class="text-[9px] sm:text-xs font-semibold text-purple-600 uppercase tracking-wider">QRIS</p>
                    <h3 class="text-sm sm:text-2xl font-extrabold text-gray-800 mt-0.5">
                        <span class="text-[10px] sm:text-sm font-normal text-gray-400">Rp</span>
                        {{ number_format($summary['total_digital'], 0, ',', '.') }}
                    </h3>
                    <p class="text-[9px] sm:text-xs text-gray-400 hidden sm:block">Masuk ke rekening</p>
                    <div class="absolute right-0 top-0 h-full w-12 sm:w-16 bg-gradient-to-l from-purple-50 to-transparent opacity-50"></div>
                </div>
            </div>
                    ...
                    <div class="px-4 py-3 border-b border-gray-100 bg-gray-50/70 flex items-center justify-between gap-3">
                        ...
                        <div class="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <div class="bg-white rounded px-2 py-1 border border-green-200 text-center">
                                <p class="text-[8px] font-semibold text-green-500 uppercase">Tips</p>
                                <p class="text-xs font-bold text-green-600">Rp {{ number_format($capsterData['total_tips'], 0, ',', '.') }}</p>
                            </div>
                            @endif
                            @if($capsterData['total_expenses'] > 0)
                            <div class="bg-white rounded px-2 py-1 border border-red-200 text-center hidden sm:block">
