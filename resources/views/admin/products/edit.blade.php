<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-4">
                <a href="{{ route('admin.products.index') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-indigo-600 transition-colors">
                ...
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.products.update', $product->id_product) }}"> @csrf
                        @method('PUT') <div>
                            ...
                            <select id="id_store" name="id_store" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id_store }}" {{ old('id_store', $product->id_store) == $store->id_store ? 'selected' : '' }}> {{ $store->store_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('id_store')
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="product_name" id="product_name" value="{{ old('product_name', $product->product_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required> @error('product_name')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4"
                             x-data="{
                                 formattedPrice: '{{ old('price') ? number_format(old('price'), 0, ',', '.') : number_format($product->price, 0, ',', '.') }}', rawPrice: '{{ old('price', $product->price) }}', formatNumber() {
                        ...
                        <div class="mt-4">
                            ...
                            <input type="number" name="stock_available" id="stock_available" value="{{ old('stock_available', $product->stock_available) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required min="0"> @error('stock_available')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('description', $product->description) }}</textarea> @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
