<x-app-layout>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-4">
                <a href="{{ route('admin.services.index') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-indigo-600 transition-colors">
                ...
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.services.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="service_name" id="service_name" value="{{ old('service_name') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                            @error('service_name')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4" 
                             x-data="{ 
                                 ...
                                 rawPrice: '{{ old('price', '') }}',
                                 formatNumber() {
                                     // 1. Hapus semua selain angka dari input yang terlihat
                                     let raw = this.formattedPrice.replace(/[^0-9]/g, '');
                                     
                                     // 2. Simpan angka mentah itu ke input tersembunyi
                                     this.rawPrice = raw; 
                                     
                                     // 3. Format input yang terlihat dengan titik (standar Indonesia)
