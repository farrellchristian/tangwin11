        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod->id_payment_method) }}">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="method_name" class="block text-sm font-medium text-gray-700">Nama Metode</label>
                            <input type="text" name="method_name" id="method_name" value="{{ old('method_name', $paymentMethod->method_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required autofocus>
                            @error('method_name')
