<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- 1. HEADER STATISTIK (DASHBOARD STYLE) --}}
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 mt-6">

                {{-- Card 1: Jadwal Aktif Hari Ini --}}
                <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-2xl p-6 text-white shadow-lg group">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-xs font-bold uppercase tracking-widest text-indigo-100">Shift Hari Ini</span>
                        </div>
                        <div class="flex items-end gap-2">
                            <h3 class="text-4xl font-black">{{ $activeTodayCount }}</h3>
                            <span class="text-sm font-medium text-indigo-200 mb-1">Jadwal</span>
                        </div>
                        <p class="text-xs text-indigo-200 mt-2">Untuk hari {{ \Carbon\Carbon::now()->translatedFormat('l') }}</p>
                    </div>
                </div>

                {{-- Card 2: Total Jadwal Tersimpan --}}
                <div class="relative overflow-hidden bg-white rounded-2xl p-6 border border-slate-100 shadow-sm group hover:shadow-md transition-all">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-xs font-bold uppercase tracking-widest text-slate-400">Total Master</span>
                        </div>
                        <h3 class="text-3xl font-bold text-slate-800">{{ $schedules->total() }}</h3>
                        <p class="text-xs text-slate-400 mt-2">Jadwal tersimpan di database</p>
                    </div>
                </div>

                {{-- Card 3: Pintasan Cepat --}}
                <div class="relative overflow-hidden bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center items-start">
                    <h4 class="font-bold text-slate-800 mb-2">Kelola Jadwal</h4>
                    <p class="text-xs text-slate-500 mb-4">Atur jam masuk dan pulang karyawan per toko.</p>
                    <a href="{{ route('admin.presence-schedules.create') }}" class="w-full inline-flex justify-center items-center px-4 py-3 bg-slate-900 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:bg-slate-800 transition shadow-lg shadow-slate-900/20">
            ...
            {{-- 2. TOOLBAR FILTER (DESAIN MELAYANG) --}}
            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                    <div>
                        <h3 class="font-bold text-slate-800 text-lg">Daftar Jadwal</h3>
                        <p class="text-xs text-slate-500">Mengatur jam kerja karyawan per cabang.</p>
                    </div>
                    {{-- Indikator Hari Ini --}}
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-xs font-bold text-indigo-700">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
                        </span>
                        Hari Ini: {{ \Carbon\Carbon::now()->translatedFormat('l') }}
                    </span>
                </div>
