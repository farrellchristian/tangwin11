            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-gray-100">
                <div class="p-4 sm:p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.employees.update', $employee->id_employee) }}"> @csrf
                        @method('PUT') <div>
                            <label for="id_store" class="block text-sm font-medium text-gray-700">Toko
                                Penempatan</label>
                            <select id="id_store" name="id_store"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="">Pilih Toko</option>
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id_store }}" {{ old('id_store', $employee->id_store) == $store->id_store ? 'selected' : '' }}>
                                        {{ $store->store_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="tel" name="phone_number" id="phone_number"
                                ...
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                placeholder="08..."> @error('phone_number')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="date" name="join_date" id="join_date"
                                ...
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                required> @error('join_date')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        </div>

                        <div class="mt-4"> <label for="exit_date"
                                class="block text-sm font-medium text-gray-700">Tanggal Keluar (Opsional)</label>
                        ...
                        {{-- <div class="mt-4">
                            <label for="photo_path" class="block text-sm font-medium text-gray-700">Foto
                                (Opsional)</label>
                            ...
                            <img src="{{ asset('storage/' . $employee->photo_path) }}"
                                alt="Foto {{ $employee->employee_name }}" class="mt-2 h-20 w-20 object-cover rounded">
                            @endif
                            @error('photo_path')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
